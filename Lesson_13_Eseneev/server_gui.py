...
class ConfigWindow(QDialog):
    ...
    def initUI(self):
        ...
        self.db_path_label = QLabel("Путь к файлу БД сервера: ", self)
        ...
        self.db_path_select.move(305, 28)
        ...
        self.db_file_label = QLabel("Имя файла БД сервера: ", self)
        self.db_file_label.move(10, 68)
        self.db_file_label.setFixedSize(215, 15)

        # Поле для ввода имени файла
        self.db_file = QLineEdit(self)
        self.db_file.move(235, 68)
        ...
        self.port.move(235, 108)
        self.port.setFixedSize(150, 20)

        # Метка с адресом для соединений
        self.ip_label = QLabel("С какого IP принимаем соединения: ", self)
        self.ip_label.move(10, 148)
        self.ip_label.setFixedSize(215, 15)
        ...
        self.ip.move(235, 148)
        self.ip.setFixedSize(150, 20)

        # Кнопка сохранения настроек
        self.save_btn = QPushButton("Сохранить", self)
        self.save_btn.move(225, 220)

        # Кнапка закрытия окна
        self.close_button = QPushButton("Закрыть", self)
        self.close_button.move(310, 220)
        self.close_button.clicked.connect(self.close)
...
if __name__ == "__main__":
    ...
    ex = MainWindow()
    ex.statusBar().showMessage("Server testing")
    ...
    test_list.appendRow(
        [
            QStandardItem("Клиент1"),
            QStandardItem("192.168.0.1"),
            QStandardItem("8001"),
            QStandardItem(time.strftime("%d.%m.%Y %H:%M")),
        ]
    )
    test_list.appendRow(
        [
            QStandardItem("Клиент2"),
            QStandardItem("192.168.0.2"),
            QStandardItem("8002"),
            QStandardItem(time.strftime("%d.%m.%Y %H:%M")),
        ]
    )
